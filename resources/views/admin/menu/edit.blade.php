          ...
          <form class="contact-form row y-gap-30" action="{{route('admin.menu.update', ['menu' => $menu->id])}}" method="POST">
            ...
            @foreach ($langs as $key => $lang)
                <div class="col-12">
                    ...
                    <input name="translations[{{$key}}][id]" type="text" value="{{$menu->translations[$key]->id}}" style="display: none">
                    <input name="translations[{{$key}}][name]" type="text" value="{{$menu->translations[$key]->name}}" placeholder="{{"Title $lang->code"}}" >
