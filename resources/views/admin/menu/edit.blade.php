          ...
          <form class="contact-form row y-gap-30" action="{{route('admin.menu.update', ['menu' => $menu->id])}}" method="POST">
            ...
            @foreach ($langs as $key => $lang)
                <div class="col-12">
                    ...
                    <input name="translations[{{$key}}][id]" type="text" @isset($menu->translations[$key]) value="{{$menu->translations[$key]->id}}" @endisset style="display: none">
                    <input name="translations[{{$key}}][name]" type="text" @isset($menu->translations[$key]) value="{{$menu->translations[$key]->name}}" @endisset placeholder="{{"Title $lang->code"}}" >
